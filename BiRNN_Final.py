...
# Model Definition
class BiRNNAttentionModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_classes):
        super(BiRNNAttentionModel, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.birnn = nn.LSTM(embedding_dim, hidden_size, bidirectional=True, batch_first=True)
        self.attention = nn.Linear(2 * hidden_size, 1)
        self.classifier = nn.Linear(2 * hidden_size, num_classes)

    def forward(self, input_ids, attention_mask):
        embedded = self.embedding(input_ids)
        birnn_output, _ = self.birnn(embedded)
        attention_scores = torch.softmax(self.attention(birnn_output), dim=1)
        context_vector = torch.sum(attention_scores * birnn_output, dim=1)
        logits = self.classifier(context_vector)
        return logits

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = BiRNNAttentionModel(vocab_size=len(tokenizer.vocab), embedding_dim=128, hidden_size=256, num_classes=2).to(device)
